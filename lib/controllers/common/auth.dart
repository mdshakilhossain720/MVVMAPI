...
class AuthController extends StateNotifier<bool> {
  ...
  Future<CommonResponse>login({required String contract, required String password}) async {
    state = true;
    bool isSuccess = false;

    try{
      final response=await ref.read(authServiceProvider).login(contract: contract, password: password);
      state = false;
